    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <Link href="/admin/produits/nouveau">
          <Button size="sm"><Plus size={16} /> Ajouter un produit</Button>
        </Link>
      </div>
              ...
              <tbody>
                {products.map((product) => (
                  <tr key={product.id} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-500">{product.category?.name_fr || '—'}</td>
                    <td className="px-4 py-3 text-right font-medium">{formatPrice(product.price)}</td>
                    <td className="px-4 py-3 text-center">
                      <span className={cn(
                        'text-xs px-2 py-0.5 rounded-full',
                        product.stock_quantity > 10 ? 'bg-green-50 text-green-600' :
                        product.stock_quantity > 0 ? 'bg-yellow-50 text-yellow-600' :
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        {product.is_featured && <span className="text-[10px] bg-gold/10 text-gold px-1.5 py-0.5 rounded">Vedette</span>}
                        {product.is_new && <span className="text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded">Nouveau</span>}
                        {product.is_on_sale && <span className="text-[10px] bg-red-50 text-red-600 px-1.5 py-0.5 rounded">Promo</span>}
                      </div>
                    </td>
